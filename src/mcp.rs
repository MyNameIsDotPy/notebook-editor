...
fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        tool(
            ...
            schema(
                &["path"],
                json!({"path": string_prop("Workspace-relative .ipynb path"), "selection": string_prop("Cell selection; default all"), "cell_type": enum_prop(&["code", "markdown", "raw"]), "lines": string_prop("1-based line selection within each cell"), "include_outputs": bool_prop("Include raw code-cell outputs and execution count"), "include_source": bool_prop("Include source; false returns outputs only"), "max_output_chars": integer_prop("Maximum characters retained per textual output")}),
            ),
        ),
        tool(
